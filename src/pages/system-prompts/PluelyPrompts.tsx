...
const SELECTED_PLUELY_MODEL_STORAGE_KEY = "selected_lamu_model";
const SELECTED_PLUELY_PROMPT_STORAGE_KEY = "selected_lamu_prompt";

export const LamuPrompts = () => {
  const {
    setSystemPrompt,
    setSupportsImages,
    lamuApiEnabled,
  ...
  const lamuApiBlocked = isBlocked;
  const [prompts, setPrompts] = useState<LamuPrompt[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [lastUpdated, setLastUpdated] = useState<string | null>(null);
  const [selectedLamuPrompt, setSelectedLamuPrompt] =
    useState<LamuPrompt | null>(() => {
  ...
  // Watch for changes in user's selected prompt and clear Lamu selection if needed
  useEffect(() => {
    const checkUserPromptSelection = () => {
      const userSelectedPromptId = safeLocalStorage.getItem(
        STORAGE_KEYS.SELECTED_SYSTEM_PROMPT_ID
      );
      // If user has selected one of their own prompts, clear Lamu prompt selection
      if (userSelectedPromptId) {
        setSelectedLamuPrompt(null);
      }
    };
  ...
  const fetchLamuPrompts = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await invoke<LamuPromptsResponse>("fetch_prompts");
      setPrompts(response.prompts);
      if (response.last_updated) {
        setLastUpdated(response.last_updated);
      }
    } catch (err) {
      console.error("Failed to fetch Lamu prompts:", err);
      setError(
        typeof err === "string" ? err : "Failed to fetch Lamu prompts"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSelectLamuPrompt = async (prompt: LamuPrompt) => {
    if (lamuApiBlocked) {
      return;
    }
    ...
    try {
      ...
      // Save the selected Lamu prompt to local storage for persistence
      ...
      console.error("Failed to select Lamu prompt:", error);
    }
  };

  const handleCardClick = (prompt: LamuPrompt) => {
    handleSelectLamuPrompt(prompt);
  };

  const isPromptSelected = (prompt: LamuPrompt) => {
    return (
      selectedLamuPrompt?.title === prompt.title &&
      selectedLamuPrompt?.modelId === prompt.modelId
    );
  };
    ...
    return (
      <div className="space-y-4 mt-6">
        ...
        <Empty
          ...
          description="Fetching Lamu default prompts"
        />
      </div>
    );
  }

  // If the backend is unreachable or returns an error, silently hide this section
  if (error) {
    return null;
  }
          ...
          return (
            <Card
              ...
              className={`relative border lg:border-2 shadow-none p-4 pb-10 gap-0 group transition-all hover:shadow-sm ${
                !lamuApiBlocked ? "cursor-pointer" : "cursor-not-allowed"
              } ${
                isSelected
