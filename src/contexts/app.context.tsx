...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [planFeatures, setPlanFeatures] = useState<PlanFeatureKey[]>([]);
  const validFeatureKeys = new Set(PLAN_FEATURES.map((f) => f.key));
  ...
  const [syncEnabled, setSyncEnabledState] = useState<boolean>(() => {
    return safeLocalStorage.getItem(STORAGE_KEYS.CONVERSATION_SYNC) === "true";
  });

  const setSyncEnabled = (enabled: boolean) => {
    setSyncEnabledState(enabled);
    safeLocalStorage.setItem(STORAGE_KEYS.CONVERSATION_SYNC, String(enabled));
    if (enabled) {
      safeLocalStorage.setItem(STORAGE_KEYS.CONVERSATION_SYNC_CONSENT, "true");
    }
  };

  // ── User identity ────────────────────────────────────────────────────────────
  const [userName, setUserNameState] = useState<string>(
    () => safeLocalStorage.getItem(STORAGE_KEYS.USER_NAME) || ""
  );

  const setUserName = (name: string) => {
    setUserNameState(name);
    safeLocalStorage.setItem(STORAGE_KEYS.USER_NAME, name);
  };

  // ── Trial period ─────────────────────────────────────────────────────────────
  // trial_expires_at is set by the backend (per instance_id) and cached locally
  // so the app still works when offline.
  const [trialExpiresAt, setTrialExpiresAt] = useState<number>(() => {
    const cached = safeLocalStorage.getItem(STORAGE_KEYS.TRIAL_EXPIRES_AT);
    // If no cached value, assume a fresh install → grant 48 h from now as optimistic default
    // The real value is set as soon as get_trial_status returns from the backend.
    return cached ? parseInt(cached, 10) : Date.now() + 2 * 24 * 60 * 60 * 1000;
  });

  const [trialExpired, setTrialExpired] = useState<boolean>(() => {
    const cached = safeLocalStorage.getItem(STORAGE_KEYS.TRIAL_EXPIRES_AT);
    if (!cached) return false; // no cache → assume trial still active until backend responds
    return Date.now() > parseInt(cached, 10);
  });

  // Re-check once per minute so the gate activates mid-session if the 48h elapses
  useEffect(() => {
    const id = setInterval(() => {
      setTrialExpired(Date.now() > trialExpiresAt);
    }, 60_000);
    return () => clearInterval(id);
  }, [trialExpiresAt]);

  const msLeft = Math.max(0, trialExpiresAt - Date.now());
  const trialDaysLeft = trialExpired
    ? 0
  ...
  // Lamu API State
  const [lamuApiEnabled, setLamuApiEnabledState] = useState<boolean>(() => {
    const stored = safeLocalStorage.getItem(STORAGE_KEYS.LAMU_API_ENABLED);
    // Default to true for new users (null = never set)
    return stored === null ? true : stored === "true";
  });
  ...
  const getActiveLicenseStatus = async () => {
    ...
    setPlanId(response.plan_id ?? null);
    setPlanName(response.plan_name ?? null);
    setPlanFeatures(
      (response.features ?? []).filter((f): f is PlanFeatureKey =>
        validFeatureKeys.has(f as PlanFeatureKey)
      )
    );
    setMaxRequests(response.max_requests ?? null);
  ...
  const loadData = () => {
    ...
    // Load Lamu API enabled state
    const savedLamuApiEnabled = safeLocalStorage.getItem(
      STORAGE_KEYS.LAMU_API_ENABLED
    );
    if (savedLamuApiEnabled !== null) {
      setLamuApiEnabledState(savedLamuApiEnabled === "true");
    }
  ...
  // Periodic license re-validation every 10 minutes
  // Ensures a revoked license is enforced mid-session without needing a restart
  useEffect(() => {
    const INTERVAL_MS = 10 * 60 * 1000;
    const id = setInterval(() => {
      getActiveLicenseStatus().catch(() => {});
    }, INTERVAL_MS);
    return () => clearInterval(id);
  }, []);
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      // The backend records `first_seen_at` per instance_id; we cache the result
      // locally so the app keeps working while offline.
      try {
        const storedName = safeLocalStorage.getItem(STORAGE_KEYS.USER_NAME) || "";
        const trial = await invoke<{ trial_expires_at: number; is_trial_active: boolean }>(
          "get_trial_status",
          { userName: storedName }
        );
        setTrialExpiresAt(trial.trial_expires_at);
        setTrialExpired(!trial.is_trial_active);
        safeLocalStorage.setItem(STORAGE_KEYS.TRIAL_EXPIRES_AT, String(trial.trial_expires_at));
      } catch {
        // "offline" or "invalid_response" — keep the cached value already in state
      }
  ...
  useEffect(() => {
    const checkImageSupport = async () => {
      if (lamuApiEnabled) {
        // For Lamu API, check the selected model's modality
        try {
          const storage = await invoke<{
          ...
          if (storage.selected_lamu_model) {
            const model = JSON.parse(storage.selected_lamu_model);
            ...
            // No specific model selected — backend decides, assume images supported
            setSupportsImages(true);
          }
        } catch (error) {
          // On error, don't disable image features
          setSupportsImages(true);
        }
      } else {
        ...
        if (provider) {
          ...
          // Unknown provider — disable images to avoid sending unsupported data
    ...
    checkImageSupport();
  }, [lamuApiEnabled, selectedAIProvider.provider]);
  ...
  const setLamuApiEnabled = async (enabled: boolean) => {
    setLamuApiEnabledState(enabled);
    safeLocalStorage.setItem(STORAGE_KEYS.LAMU_API_ENABLED, String(enabled));
    ...
    if (enabled) {
      try {
        ...
        console.debug("Failed to check Lamu model image support:", error);
        setSupportsImages(false);
      }
    } else {
  ...
  const value: IContextType = {
    ...
    planFeatures,
    maxRequests,
    hasPlanFeature: (feature: PlanFeatureKey) => planFeatures.includes(feature),
    priceLabel,
