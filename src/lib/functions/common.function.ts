...
export function buildDynamicMessages(
  ...
  // Normalize content: if content is an array with only text items (no images),
  // collapse to a plain string so providers like Claude don't reject it.
  const normalizeMessage = (msg: any) => {
    if (
      msg?.content &&
      Array.isArray(msg.content) &&
      msg.content.every((item: any) => item?.type === "text") &&
      !msg.content.some((item: any) => item?.type === "image" || item?.type === "image_url")
    ) {
      return { ...msg, content: msg.content.map((item: any) => item.text).join("") };
    }
    return msg;
  };

  const messages = [...prefixMessages, ...history, newUserMessage, ...suffixMessages];
