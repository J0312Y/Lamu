...
// Lamu STT function
async function fetchLamuSTT(audio: File | Blob, language?: string): Promise<string> {
  try {
    ...
    return `Lamu STT Error: ${errorMessage}`;
  }
}
...
export interface STTParams {
  ...
  audio: File | Blob;
  /** BCP-47 language code (e.g. "fr", "en", "es"). Omit or pass undefined for auto-detection. */
  language?: string;
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    if (useLamuAPI) {
      return await fetchLamuSTT(audio, params.language);
    }
    ...
    if (isForm) {
      ...
      // Inject language hint if provided (Whisper supports BCP-47 codes)
      if (params.language) {
