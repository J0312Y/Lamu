// Lamu AI Speech Detection, and capture system audio (speaker output) as a stream of f32 samples.
...
impl Default for VadConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            hop_size: 1024,
            sensitivity_rms: 0.04,  // High threshold — only clear voice triggers detection
            peak_threshold: 0.08,   // High peak threshold
            silence_chunks: 20,     // ~0.46s of silence before stopping
            min_speech_chunks: 5,   // ~0.11s minimum speech
            pre_speech_chunks: 10,  // ~0.23s pre-speech buffer
            noise_gate_threshold: 0.015, // Hard gate — zeros out all background noise
...
async fn run_vad_capture(
    ...
    // Streaming transcription: emit a WAV snapshot every 2s while in_speech
        ...
        while buffer.len() >= config.hop_size {
            ...
            if is_speech {
                if !in_speech {
                    ...
                    let _ = app.emit("speech-in-progress", 0u32);
                }
                ...
                // Emit live progress every ~500ms while collecting speech
                if last_progress_emit.elapsed() >= Duration::from_millis(500) {
                    last_progress_emit = Instant::now();
                    let elapsed_ms = speech_start
                        .map(|s| s.elapsed().as_millis() as u32)
                        .unwrap_or(0);
                    let _ = app.emit("speech-in-progress", elapsed_ms);
                }
                ...
                if speech_buffer.len() > max_samples {
                    ...
                    speech_chunks = 0;
                    speech_start = None;
                    let _ = app.emit("speech-in-progress", u32::MAX); // sentinel: done
                }
            } else {
                // Silence detected
                if in_speech {
            ...
            // Streaming transcription: every 2s while collecting speech, emit a WAV
            // snapshot so the frontend can run a background STT pass for live captions.
            // Minimum 1s of audio (sr samples) to avoid sending near-empty clips.
            if in_speech
                && last_chunk_emit.elapsed() >= Duration::from_secs(2)
                && speech_buffer.len() >= sr as usize
            {
                last_chunk_emit = Instant::now();
                let snapshot = normalize_audio_level(&speech_buffer, 0.1);
                if let Ok(b64) = samples_to_wav_b64(sr, &snapshot) {
...
async fn run_continuous_capture(
    ...
    if !audio_buffer.is_empty() {
        ...
        match samples_to_wav_b64(sr, &cleaned_audio) {
            Ok(b64) => {
                // Emit a distinct event so the frontend can log mic speech
                // separately from system audio (meeting transcript "me" role)
...
fn apply_noise_gate(samples: &[f32], threshold: f32) -> Vec<f32> {
    // Hard gate: zero out anything below threshold to eliminate ambient noise
    samples
        .iter()
...
pub async fn start_mic_capture(
    ...
    // Prevent double-start
    {
        let guard = state
            .mic_task
            .lock()
            .map_err(|e| format!("Mic lock error: {}", e))?;
        if guard.is_some() {
            return Err("Mic capture already running".to_string());
        }
    }

    let config = if let Some(c) = vad_config {
        c
    } else {
        state
            .vad_config
    ...
    let mic_stream = crate::speaker::mic::MicStream::new(device_id)
        .map_err(|e| format!("Failed to open mic: {}", e))?;
    let sr = mic_stream.sample_rate();

    let app_clone = app.clone();
    let task = tokio::spawn(async move {
        if config.enabled {
            run_vad_capture(app_clone, mic_stream, sr, config).await;
        } else {
            run_continuous_capture(app_clone, mic_stream, sr, config).await;
        }
    });
    ...
    *state
        .mic_task
        .lock()
        .map_err(|e| format!("Mic lock error: {}", e))? = Some(task);
...
pub async fn stop_mic_capture(app: AppHandle) -> Result<(), String> {
    ...
    // Drop the guard before awaiting to keep the future Send
    {
        let mut guard = state
            .mic_task
            .lock()
...
#[tauri::command]
pub fn check_mic_access() -> Result<bool, String> {
    match crate::speaker::mic::MicStream::new(None) {
        Ok(_) => Ok(true),
        Err(e) => {
            error!("Mic access check failed: {}", e);
            Ok(false)
        }
    }
}
