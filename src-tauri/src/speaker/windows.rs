// Lamu windows speaker input and stream
...
impl SpeakerInput {
    ...
    pub fn stream(self) -> SpeakerStream {
        ...
        let capture_thread = thread::spawn(move || {
            ...
            {
                error!("Lamu Audio capture loop failed: {}", e);
            }
        });
        ...
        let actual_sample_rate = match init_rx.recv_timeout(Duration::from_secs(5)) {
            Ok(Ok(rate)) => rate,
            Ok(Err(e)) => {
                error!("Lamu Audio initialization failed: {}", e);
                44100
            }
            Err(_) => {
                error!("Lamu Audio initialization timeout");
                44100
            }
        };
            ...
            Ok((h_event, render_client, sample_rate)) => {
                ...
                loop {
                    ...
                    if h_event.wait_for_event(3000).is_err() {
                        error!("Lamu timeout error, stopping capture");
                        break;
                    }

                    let mut temp_queue = VecDeque::new();
                    if let Err(e) = render_client.read_from_device_to_deque(&mut temp_queue) {
                        error!("Lamu Failed to read audio data: {}", e);
                        continue;
                    }
