...
// ── Windows: exclude overlay from screen-capture APIs ─────────────────────────
// SetWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE = 0x11) makes the window
// completely invisible in Zoom/Teams/Meet screen shares and screenshots, while
// remaining visible to the user.  This supersedes contentProtected (WDA_MONITOR)
// which turns the window black instead of invisible.
#[cfg(target_os = "windows")]
pub fn apply_screen_capture_exclusion(window: &WebviewWindow) {
    use raw_window_handle::{HasWindowHandle, RawWindowHandle};
    if let Ok(handle) = window.window_handle() {
        if let RawWindowHandle::Win32(win32) = handle.as_raw() {
            // windows-sys defines HWND as *mut c_void
...
pub fn setup_main_window(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let window = app
        ...
        .or_else(|| app.get_webview_window("lamu"))
        .or_else(|| {
    ...
    // On Windows: override contentProtected (WDA_MONITOR = black) with
    // WDA_EXCLUDEFROMCAPTURE so the overlay is invisible—not black—in screen shares.
    #[cfg(target_os = "windows")]
