...
pub fn handle_shortcut_action<R: Runtime>(app: &AppHandle<R>, action_id: &str) {
    match action_id {
        ...
        "toggle_mic" => handle_toggle_mic_shortcut(app),
        custom_action => {
...
/// Handle mic toggle shortcut
fn handle_toggle_mic_shortcut<R: Runtime>(app: &AppHandle<R>) {
    if let Some(window) = app.get_webview_window("main") {
        if let Ok(false) = window.is_visible() {
            let _ = window.show();
        }
        if let Err(e) = window.emit("toggle-mic-capture", json!({})) {
            eprintln!("Failed to emit toggle-mic-capture event: {}", e);
        }
    }
}
...
pub fn update_shortcuts<R: Runtime>(
    ...
    // Unregister ALL OS-level shortcuts unconditionally so stale shortcuts
    // from a previous session (or a previous update_shortcuts call whose state
    // got out of sync) don't cause "HotKey already registered" failures.
    if let Err(e) = app.global_shortcut().unregister_all() {
        eprintln!("Failed to unregister_all shortcuts: {}", e);
    }
